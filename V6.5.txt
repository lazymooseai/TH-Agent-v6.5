...
def tunnista_terminaali(teksti, laiva_nimi="", aika=""):
    ...
    # PDF Ohje: Eckerö Line MS Finlandia saapuu Länsisatamaan T2
...
# ==========================================
# HAKUFUNKTIOT: TAPAHTUMAT (LINKED EVENTS API & LIIGA)
# ==========================================
@st.cache_data(ttl=86400)
def hae_paikka_id(hakusana: str) -> str:
    """Hakee paikan ID:n Helsingin Linked Events API:sta nimen perusteella."""
...
def hae_tapahtumat_api(paikka_id: str, pvm_iso: str) -> list:
    """Hakee tietyn paikan tapahtumat tietylle päivälle."""
    if not paikka_id:
        return []
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for t in data:
                ...
                alku = t.get("start_time", "")
                klo = alku[11:16] if len(alku) > 10 else ""
                if klo:
                    tulos.append(f"{nimi} klo {klo}")
                else:
                    tulos.append(nimi)
            return tulos
    except:
...
def yhdista_kulttuuridata(paikat, pvm_iso: str):
    for p in paikat:
        ...
        # Haetaan API:sta, jos hakusana on määritetty
        if hakusanat:
        ...
        if tapahtumat:
            p["lopetus_html"] = f"<span class='live-event'>ESITYKSIÄ TÄNÄÄN: {', '.join(tapahtumat)}</span>"
        else:
            p["lopetus_html"] = (
...
def yhdista_urheiludata(paikat, pvm_iso: str):
    ...
    for p in paikat:
        ...
        if "hifk" in nimi:
            ...
            # Mestis poikkeus PDF:n mukaan
            p["lopetus_html"] = "<span class='no-event'>Mestis-kalenteri ei suorassa haussa. <a href='https://jokerit.fi/ottelut' target='_blank' style='color:#5bc0de;'>Tarkista Jokereiden sivuilta →</a></span>"
            continue
...
def render_dashboard():
    ...
    with tab1:
        # Hakusanat on optimoitu Helsingin Linked Events API:a varten
        kulttuuri_paikat = [
