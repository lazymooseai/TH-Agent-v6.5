...
def get_trains(asema_nimi):
    ...
    try:
        ...
        for juna in junat:
            ...
            # KORJAUS 1: Suodatetaan pois junat, jotka LÄHTEVÄT pääkaupunkiseudulta (menevät poispäin)
...
def parse_hel_api_time(time_str):
    """Muuttaa API:n UTC-ajan Helsingin aikaan ja palauttaa muodossa HH:MM"""
    if not time_str:
        return None
    try:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for t in data:
                ...
                # KORJAUS 2: Haetaan alkamis- ja päättymisaika ja muutetaan se paikalliseen aikaan
                ...
                # Muotoillaan jokainen tapahtuma siistiksi HTML-riviksi
...
def yhdista_kulttuuridata(paikat, pvm_iso: str):
    for p in paikat:
        ...
        if tapahtumat:
            # KORJAUS 2: Tapahtumat esitetään nyt listana allekkain (ei enää yhtenä pötkönä)
