...
# ==========================================
# HAKUFUNKTIOT: TAPAHTUMAT (KORJATTU SUODATUS)
# ==========================================
...
def hae_paikka_id(hakusana: str) -> str:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if data:
                # Kerätään max 3 osumaa siltä varalta, että talossa on useita saleja (Päänäyttämö, Alminsali jne.)
                return ",".join([p["id"] for p in data[:3]])
    except:
        pass
    return ""
...
def hae_tapahtumat_api(paikka_id: str, pvm_iso: str) -> list:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for t in data:
                # 1. SUODATIN: Ohitetaan koko kauden kattavat "umbrella"- ja "recurring"-tapahtumat
                super_type = t.get("super_event_type")
                if super_type in ["umbrella", "recurring"]:
                    continue
                
                # 2. SUODATIN: Varmistetaan, ettei tapahtumaa ole peruttu
                status = t.get("event_status")
                if status and status != "EventScheduled":
                    continue

                # 3. SUODATIN: Tarkistetaan, että tapahtuma oikeasti ALKAA valittuna päivänä
                start_str = t.get("start_time", "")
                if not start_str:
                    continue # Ei sellaista taksikeikkaa olekaan, jolla ei ole kellonaikaa!
                    
                start_dt = datetime.datetime.fromisoformat(start_str.replace("Z", "+00:00")).astimezone(ZoneInfo("Europe/Helsinki"))
                if start_dt.strftime("%Y-%m-%d") != pvm_iso:
                    continue # Esimerkiksi kuukausia kestävä näyttely poistuu tässä.

                alku_klo = start_dt.strftime("%H:%M")
            ...
            # Poistetaan täsmälleen samanlaiset duplikaatit säilyttäen järjestys
            uniikit = []
            seen = set()
            for x in tulos:
                if x not in seen:
...
def yhdista_kulttuuridata(paikat, pvm_iso: str):
    for p in paikat:
        ...
        if tapahtumat:
            ...
            p["lopetus_html"] = (
                f"<span class='no-event'>Ei havaittuja esityksiä tälle illalle.</span>"
