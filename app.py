...
<style>
    ...
    .event-list-item { font-size: 17px; color: #d0d0d0; margin-bottom: 6px; line-height: 1.3;}
    .tomorrow-banner {
...
def get_trains(asema_nimi):
    ...
    try:
        ...
        for juna in junat:
            ...
            t_rows = juna.get("timeTableRows", [])
            if not t_rows:
                continue
                
            # Pomminvarma tarkistus: Haetaan junan alkuperäinen lähtöasema (reitin ensimmäinen asema)
            todellinen_lahto_koodi = t_rows[0].get("stationShortCode")

            # KORJAUS 1: Suodatetaan pois KAIKKI junat, jotka LÄHTEVÄT pääkaupunkiseudulta
            if not todellinen_lahto_koodi or todellinen_lahto_koodi in ["HKI", "PSL", "TKL"]:
                continue
            ...
            if aika_str and aika_obj_hki:
                tulos.append({
                    "train": nimi,
                    "origin": asemat_dict.get(todellinen_lahto_koodi, todellinen_lahto_koodi),
                    "time": aika_str,
                    "delay": viive,
                    "dt": aika_obj_hki
                })
...
# ==========================================
# HAKUFUNKTIOT: TAPAHTUMAT (KORJATTU AIKA & ULKOASU)
# ==========================================
...
def hae_tapahtumat_api(paikka_id: str, pvm_iso: str) -> list:
    ...
    try:
        ...
        if r.status_code == 200:
            data = r.json().get("data", [])
            tulos = []
            kohde_pvm = datetime.datetime.strptime(pvm_iso, "%Y-%m-%d").date()
            
            for t in data:
                nimi = (t.get("name", {}) or {}).get("fi", "Esitys")
                
                alku_dt = parse_hel_api_time(t.get("start_time"))
                loppu_dt = parse_hel_api_time(t.get("end_time"))
                
                if not alku_dt:
                    continue
                
                # KORJAUS 2A: Ohitetaan näyttelyt yms. pitkät tapahtumat, jotka EIVÄT ala juuri tänään
                if alku_dt.date() != kohde_pvm:
                    continue
                
                alku_klo = alku_dt.strftime("%H:%M")
                
                # KORJAUS 2B: Näytetään päättymisaika VAIN jos se on loogisesti saman päivän sisällä
                if loppu_dt and loppu_dt.date() == alku_dt.date():
                    loppu_klo = loppu_dt.strftime("%H:%M")
...
def yhdista_kulttuuridata(paikat, pvm_iso: str):
    for p in paikat:
        ...
        if tapahtumat:
            ...
            p["lopetus_html"] = (
                f"<span class='no-event'>Ei tänään alkavia esityksiä.</span>"
                f"<br><span style='color:#777;font-size:15px;'>Tyypillisesti: {p.get('huomio','')}</span>"
            )
    return paikat
